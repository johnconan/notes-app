<template>
  <div class="notes">
    <div class="note" :class="[{ full: !grid }, note.notify]" v-for="(note, index) in notes" :key="index">
      ...
      <p v-if="!note.edit" @click="$emit('edit', index)" class="notify">{{ note.title }}</p>

      <input
        @keydown.enter="$emit('saveEdit', index)"
        @keydown.esc="$emit('cancelEdit', index)" 
        v-if="note.edit" type="text" 
        v-model="editFields.editTitle" >

        <p style="cursor: pointer;"
           @click="$emit('remove', index)">x</p>
      </div>
